...
void evaluate_load(double g, int n, int n_rx,int d[],
		double Lambda[],  int q,  int iter_max,
		int how_often_to_decode,  bool boundary_effect,
		bool first_slot_tx,  int num_packets_to_sim,
		int num_PL_to_sim, vector<double> &PLR,int index,
		int maximum_delay, vector<vector<int> > &delays,
		bool save_delays, bool progression,
		vector<int> &load_tracker,vector<int> &resolve_tracker,
...
int main () {
	...
	int n,n_rx,maximum_delay,num_packets_to_sim,num_PL_to_sim,q,
	iter_max,how_often_to_decode, g_len, g_index;
	...
	string filename =
			"FA-CSA-3-" + to_string(n) + "n_rx_" + to_string(n_rx)+"max_delay_" +
			to_string(maximum_delay) +"_BE_"+ to_string(boundary_effect)+"_firstslottx_" +
			to_string(first_slot_tx)+ "_singleton_tracker_g_083.txt";
	...
	while(g_index>=0)
	{
		evaluate_load(g[g_index], n, n_rx, d, Lambda,
				q, iter_max, how_often_to_decode,
				boundary_effect, first_slot_tx,
				num_packets_to_sim, num_PL_to_sim,
				PLR, g_index, maximum_delay, delays,
				save_delays, save_progression,
				load_tracker,resolve_tracker,
				singleton_tracker);
		g_index--;
	}
	...
	if(save_progression){
		output_file << endl;
		output_file << endl;
		output_file <<"Load tracker (number of active users over time)  size:"
				<< load_tracker.size() <<endl;
		...
		output_file << endl;
		output_file << endl;
		output_file <<"Resolve tracker (number of resolved-active users over time)  size:"
				<< resolve_tracker.size() <<endl;
		...
		output_file << endl;
		output_file << endl;
		output_file <<"Singleton tracker (number of singleton slots over time)  size:"
				<< singleton_tracker.size() <<endl;
...
void evaluate_load(double g, int n, int n_rx,int d[],  double Lambda[],  int q,  int iter_max,  int how_often_to_decode,  bool boundary_effect,  bool first_slot_tx,  int num_packets_to_sim, int num_PL_to_sim, vector<double> &PLR,int index, int maximum_delay, vector<vector<int> > &delays, bool save_delays, bool save_progression,vector<int> &load_tracker,vector<int> &resolve_tracker, vector<int> &singleton_tracker)
{
	...
	cout<< g << " \t \t "<< setw((int)log10(num_packets_to_sim)+1) <<
	packets_sent << " \t \t" << setw((int)log10(num_PL_to_sim)+1) <<
	packets_lost << " \t \t "<< scientific<<PLR.at(index)<<endl;
